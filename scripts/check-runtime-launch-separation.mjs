...
const apiLauncher = readFileSync(path.join(root, "scripts/run-api.cmd"), "utf8");
...
const runtimeEntrypoint = readFileSync(path.join(root, "runtime/dashboardRuntime.ts"), "utf8");
...
assert.equal(packageJson.scripts["start:api"], "scripts\\run-api.cmd start");
assert.equal(packageJson.scripts["dev:api"], "scripts\\run-api.cmd dev");
...
assert.match(apiLauncher, /-Role api/);
assert.match(apiLauncher, /Dashboard pages and static tools are disabled/);
assert.equal(packageJson.scripts["start:studio"], undefined);
assert.equal(packageJson.scripts["dev:studio"], undefined);
assert.match(roleLauncher, /DASHBOARD_ENABLED = "false"/);
assert.match(roleLauncher, /DASHBOARD_UI_ENABLED = "false"/);
assert.match(roleLauncher, /"api:start"/);
assert.match(roleLauncher, /"api:dev"/);
...
assert.doesNotMatch(runtimeEntrypoint, /DASHBOARD_ENABLED\s*=\s*"true"/);
assert.match(runtimeEntrypoint, /run-server\.cmd deliberately runs headless/);
