...
- `GET /api/llm-tools` returns the authenticated tool-calling manifest. It includes image, image-to-3D, audio, music, video, tool-resource, artifact-recovery, and generic server-tool invocation functions. Its `tools` collection labels every catalog tool as `client` or `server`; only registered server tools can be called through `POST /api/tools/invoke` with an exact `toolId` plus `input`. Client tools remain usable in the Dashboard and may receive resources, but the server never pretends it executed them.
...
A game, app, or LLM integration needs only a running URageNow release server as an external HTTP service; it does not need this repository and must not infer server absence from its own project files. Configure that project's URageNow API base URL and authorization, then request `GET {baseUrl}/api/llm-tools` to discover the functions and their current schemas. Call those endpoints with a normal HTTP client — do not launch a browser or use Playwright unless you are explicitly testing the Dashboard UI. The portable skill includes copyable health, manifest, generation, download, recovery, and shell-safe JSON-file commands through a dependency-free Node 18+ client for Windows, macOS, and Linux, plus a Windows PowerShell alternative. The `.agents/skills/URageNow` folder can be copied into a project whose agent/IDE supports project-local skills. Loading that skill only provides instructions: the client must immediately execute its `health` helper command, then `manifest`, and must not reload the skill or inspect its workspace as a substitute for those API calls. An agent must execute the next matching helper command rather than treating narration or command examples as API progress; for a requested image, it saves the returned artifact with `download` and reports the output path.
...
- The release API can now invoke Pixel Art Converter, Normalmap Maker, and Image To Ascii against an existing URage image artifact. The live tool manifest identifies these as server-capable transformations; callers preserve the source image ID and file name rather than substituting image generation.
