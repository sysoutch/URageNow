...
A game, app, or LLM integration needs only a running URageNow release server as an external HTTP service; it does not need this repository and must not infer server absence from its own project files. Configure that project's URageNow API base URL and authorization, then request `GET {baseUrl}/api/llm-tools` to discover the functions and their current schemas. Call those endpoints with a normal HTTP client — do not launch a browser or use Playwright unless you are explicitly testing the Dashboard UI. The portable skill includes copyable health, manifest, generation, download, and recovery commands through a dependency-free Node 18+ client for Windows, macOS, and Linux, plus a Windows PowerShell alternative. The `.agents/skills/URageNow` folder can be copied into a project whose agent/IDE supports project-local skills.
