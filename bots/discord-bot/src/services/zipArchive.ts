...
function sanitizeZipEntryName(value: string): string {
  return String(value || "file.bin").replace(/\\/g, "/").split("/").map(part =>
    part.replace(/[^a-zA-Z0-9._-]+/g, "_").replace(/^_+/, "") || "file"
  ).join("/");
}

export function createStoredZipArchive(entries: Array<{ name: string; data: Buffer; }>): Buffer {
