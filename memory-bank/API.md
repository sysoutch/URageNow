...
Use `POST /api/image-import` with an image `dataUrl` to put an external attachment into generated image history; preserve its returned `id` and `imageFileName` when calling a named transformation. Both shipped helpers accept ordinary JSON files with CRLF or LF endings, and the Sharp-backed converters accept PNG as well as JPEG; JSON parsing failures normally indicate malformed or shell-mangled payloads. When an agent has a local attachment path, use the helpers' `import-image-file` action instead of creating a base64 JSON file. Metadata stripping preserves animated imports by re-encoding them as GIF instead of flattening them to PNG. The copied URageNow skill also ships a stdio MCP bridge that exposes the named image transformations as callable tools; its one-step installers register a path-free `uragenow-mcp` command for Cline and Codex. The Codex installer resolves `%USERPROFILE%\.codex\config.toml` at install time and changes only its `[mcp_servers.uragenow]` section. Pixel Art Converter uses two separate Sharp resize passes because a second resize on one Sharp pipeline replaces the first; converter callers must return the new artifact record rather than the imported source. Use the MCP bridge as the primary LLM interface: it exposes media generation, server tools, artifact downloads, recovery, and tool-resource handoffs. Its initialization instructions carry the no-browser and exact-named-tool rules. Keep the reduced skill only for installation and direct-HTTP fallback. Use `GET /api/llm-tools` to discover catalog capabilities: each tool is marked `client` or `server`. Client tools remain Dashboard-only and can receive persistent resources but are not executed by the API. A registered server adapter is invoked through `POST /api/tools/invoke` with its exact `toolId` and `input`; this prevents a named tool request from silently being replaced with generic media generation. A tool can opt into an approved server action by placing a declarative `server-tool.json` beside its client files; the server discovers that manifest at runtime and advertises it even when the tool was not added to the central catalog. Manifests never execute code: new action implementations remain trusted neutral server code in server/src/tool-actions, while Dashboard owns only HTTP-route adaptation and persistence hooks. This keeps a copied or third-party tool from gaining arbitrary server access. Pixel Art Converter, Normalmap Maker, Image To Ascii, Color Palette Extractor, Color Swapper, Image Transparency Tool, Image Crop And Scale, Ai Image Naturalizer, Toon Image Shader, Qr Code Creator, Base64 Converter, Image Metadata Remover, Meta Og Tag Generator, Favicon Creator, Image Split And Combine, Pseudo Albedo Tools, Html Button Generator, Css Background Generator, C# Class Extractor, Css To Scss, Seamless Texture Maker, Gamejuice Generator, and Image To Svg Converter are server adapters. Viewers, recorders, games, and interactive browser editors intentionally remain client-only until their workflows have a safe server implementation. Image-transform tools take a URage image `imageId` and `imageFileName`; Pixel Art additionally accepts `pixelSize`, Normalmap Maker accepts `strength`, and Image To Ascii accepts `columns`, `characterSet`, and `colorMode`, defaults to Detailed/Original colors, and returns its ASCII text with a rendered PNG (static source) or animated GIF (animated GIF source) artifact. Qr Code Creator takes `text` and optional `size` and returns a standard generated-image artifact. Base64 Converter safely encodes UTF-8 text or decodes Base64, Base64URL, and Base64 data URLs to UTF-8 text; binary-file conversion remains client-side for now. Image Metadata Remover accepts a generated image record and creates a metadata-free PNG, JPEG, or WebP artifact. Meta Og Tag Generator returns escaped Open Graph and Twitter card HTML from a title, description, canonical URL, and image URL. Favicon Creator returns five standard downloadable PNG icon artifacts plus a `site.webmanifest`; it does not pretend that the generated-image library is a ZIP archive. Image Split And Combine currently exposes its bounded split operation (up to 64 PNG/JPEG/WebP tiles) as server artifacts; combining multiple independent images remains client-side until a multi-source request contract is added. Pseudo Albedo Tools creates the same deliberately stylized (not physically recovered) albedo PNG from a normal or height map, with sources limited to 2048px per side for predictable server cost. Html Button Generator returns escaped button HTML and validated CSS for solid or gradient styles. Css Background Generator returns validated linear, radial, or conic gradient CSS. C# Class Extractor returns individually named C# class, struct, and enum source files from bounded source text. Css To Scss returns categorized SCSS partials plus its main import manifest; it can extract color and spacing variables but does not compile Sass. Seamless Texture Maker exposes only a deterministic mirrored tile server action; its noise mask and interactive 3D preview remain client-side. Gamejuice Generator returns deterministic action timing, particle, shake, and sprite-selection preset data; its animated preview and procedural PNG sprite downloads remain client-side. Image To Svg Converter reuses the bundled ImageTracer implementation and persists a downloadable SVG for source images up to one megapixel, with bounded smoothing and palette options. Css To Scss persists every generated SCSS module, and Gamejuice Generator persists its preset JSON. These non-media results use the shared data/tool-artifacts store and return artifact id, fileName, MIME type, byteLength, and a relative downloadUrl; retrieve them through GET /api/tool-artifact?artifactId=<id>&file=<fileName> or the MCP urage_download_tool_artifact operation. Spritesheet Utility exposes regular-grid slicing only: it returns up to 64 standard generated-image sprite artifacts plus a persistent atlas JSON. Its interactive guide editing, diagonal splitting, and AI boundary detection remain client-only. Dialogue Tree Visualizer accepts and validates its native variables, nodes, and connections JSON before persisting urage_engine_data.json; it does not infer story content or render the interactive graph. Roadmap Builder accepts and validates its native milestones JSON before persisting roadmap.json; it does not simulate the browser editor. Html Separator And Combiner separates inline HTML into independent HTML, CSS, and JavaScript text outputs or combines them back into one document.
...
`npm run start:api` is the API-only profile. It reuses the Dashboard HTTP listener, route groups, authentication, and persistence while setting `DASHBOARD_UI_ENABLED=false`, disabling Dashboard/static-tool routes, and suppressing messenger autostart. It still exposes all existing `/api/*` capabilities, including the LLM manifest, persistent tool-resource handoff, and persistent tool-artifact downloads. `npm run start:server` is intentionally not an API host.
