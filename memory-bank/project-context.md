...
- The user-facing product name is **URage Now**: `U` uses a neutral metallic gradient, `RAGE` inherits the active theme gradient, and `Now` adapts to the theme contrast. Internal runtime, package, and repository identifiers remain `URageStudio` until a compatibility-aware migration is explicitly planned.
...
- Android companion workflow errors use stable response codes. `vision_model_required` is a user-actionable `422` for image interpretation when the selected model cannot accept images; `companion_permission_denied` is a `403` that identifies the blocked Studio capability and directs the dashboard owner to **Settings > Network > Paired devices**. Do not expose provider payloads or stack traces to mobile users.
...
- Game Engines -> Projects cards render their own per-engine brand icon (`unity.svg`, `godot.svg`, or `unreal.svg` from `dashboard/assets/game-engines/`, served at `/assets/game-engines/<engine>.svg`) instead of cloning one Unity glyph for every engine.
- Game Engines -> Projects keeps Unity Hub discovery strictly on the Unity selection. Browse, Scan, and Fetch carry the active engine; Godot discovers `project.godot` projects and its Project Manager catalog, while Unreal discovers `.uproject` projects and recent-project settings.
...
# Art tool responsive QA (2026-09-20)

- `scripts/audit-tool-screenshots.mjs` now captures phone (390px), tablet (900px), and desktop (1440px) viewport screenshots. It optionally accepts a category and comma-separated tool slugs so a focused suite can complete even when an unrelated tool is slow. It records browser failures, console errors, and horizontal overflow; use viewport captures for responsive assessment rather than unbounded full-page screenshots.
- Verified Art tool pass: all 20 Art tools were audited at all three viewport sizes in four focused browser batches with zero page-load failures, horizontal overflow, or console errors. Responsive workspace repairs are in 3D Model Viewer, SVG Editor, Seamless Texture Maker, Texture Map Painter, Image to SVG Converter, Color Swapper, Image Crop & Scale, and Pixel Art Converter. On phones, these tools use a stacked/document-scrolling flow so fixed inspectors never collapse the interactive canvas to an unusable sliver.
- `npm.cmd run check:tools` currently fails in the unrelated `check-remote-blender-script-catalog.mjs`: it expects `Local Starter Collections` in the Blender catalog template, but that text is absent. This was observed during the responsive pass and is not caused by the Art-tool CSS or audit changes.
# URageNow server-tool modularization (2026-09-20)

- The URageNow MCP `urage_split_html_document` action must be the path for splitting a self-contained tool. Download each returned persistent artifact by its exact `artifactId`, `fileName`, and a new `outputPath`; validate the temporary index links `style.css` and `script.js` before replacing the source. The server splitter preserves external scripts, import maps, all inline style blocks, and classic versus module script semantics. It rejects a document that mixes executable classic and module inline scripts rather than producing a broken page.
- Modularized and viewport-verified tools include Albedo Tools, AI Image Naturalizer, Toon Image Shader, Audio Cutter, Flowchart Builder, 3D Model Sharer, Filer, Base64 Converter, C# Class Extractor, CSS Background Generator, CSS to SCSS, and Image Metadata Remover. 3D Model Sharer is the module/import-map regression case; retain its import map and use a module script link.
# Tool responsive modularization (2026-09-20)

- Continue using the MCP split/download workflow for self-contained tools, but treat the new `style.css` as an opportunity to give each tool a breakpoint-specific workspace. Wave Animation now uses a desktop/tablet inspector and a stacked, document-scrolling phone workspace. AI Legit Matrix retains a deliberate horizontally scrollable matrix while using touch-sized, grid-organized controls on phones. Snake scales its canvas to the available width and keeps controls outside the play surface; do not reintroduce its former absolute control overlay.
- Theme-aware tool surfaces must map fixed UI colors to the shared `--tool-*` dashboard tokens. Poker Blindlevel Manager and Kanban are reference cases: status colors use semantic token-backed color mixes, while navigation, cards, inputs, borders, and active states inherit the active dashboard theme. Do not restore fixed gold, slate, white, or light-mode priority colors in those tools.
- When modularizing a standalone tool, load `tools/shared/dashboard-theme.js` in `<head>` before its `style.css`; a late loader can leave the first paint on a fixed palette. Roadmap Builder and Blender Control Center are theme-aware responsive references: Roadmap switches to a one-sided mobile timeline, and Blender Control Center maps its empty-stream surface to theme tokens while retaining its full-screen stream behavior.
- Dialogue Tree Visualizer and Normalmap Maker are further theme-aware responsive references. Dialogue’s graph background, controls, and nodes map to `--tool-*` tokens and collapse its variable manager/header into touch-safe sections. Normalmap Maker maps its shell controls to those tokens while preserving generated normal-map pixels; on narrow widths its inspector becomes a document flow with touch-sized output actions and stacked numeric/range controls.
- Image to ASCII’s shell uses shared dashboard tokens while preserving its selected output color mode. 3D Model Sharer must never declare `--tool-*` variables with `!important`: it receives the active palette from `dashboard-theme.js`. Its viewer, upload, warnings, and mobile preview should derive from that palette rather than a permanent purple/blue override.